...
public class IndexStorageBarrel extends UnicastRemoteObject implements BarrelInterface {
    ...
    public void addDocument(PageInfo page) throws RemoteException {
        ...
        //Para cada palavra (token) encontrada na página, adiciona o URL dessa página à lista de URLs associada a essa palavra.
        for (String t : page.tokens()) {
            /*
             * Procura a chave t (a palavra) no mapa index.
                Se já existir, devolve o conjunto (Set<String>) que está lá dentro.
                Se não existir, cria um novo conjunto vazio (ConcurrentHashMap.newKeySet()), mete-o no mapa e devolve-o.
             */
            index.computeIfAbsent(t, k -> ConcurrentHashMap.newKeySet()).add(page.url());
        }
        ...
        for (String out : page.outLinks()) {
            backlinks.computeIfAbsent(out, k -> ConcurrentHashMap.newKeySet()).add(page.url());
        }
    }
    ...
    @Override
    public List<SearchResult> searchAllTerms(List<String> terms, int pageIdx, int pageSize) throws RemoteException {
        ...
        for (String u : urls.subList(from, to)) {
            ...
            out.add(new SearchResult(u, p != null ? p.title() : u, snippetOf(p), indeg));
        }
        return out;
    }
    ...
    private static String snippetOf(PageInfo p) { return p != null ? p.snippet() : ""; }
